...
const GifCards = () => {
	...
	const { data, fetchNextPage, hasNextPage, isLoading, error } = useLoadMoreSearchGifs({
		q: searchParams?.q || '',
		limit: 9,
		offset: searchParams?.offset || 0,
	});

	const { data: gifDetails, isLoading: detailsLoading } = useGifDetails(selectedGif?.id || '');
		...
		return (
			<div className='text-center text-red-500'>
				...
				{error.message.includes('404') && (
					<p>
						The API server is unavailable. Please check if the server is running on
						http://localhost:5000.
					</p>
				)}
			</div>
